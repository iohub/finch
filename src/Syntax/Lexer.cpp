...
namespace Finch {
bool Lexer::IsInfinite() const { return mReader.IsInfinite(); }
Ref<Token> Lexer::ReadToken() {
  while (true) {
    if (IsDone())
      return Ref<Token>(new Token(TOKEN_EOF));
    ...
    case '(':
      return SingleToken(TOKEN_LEFT_PAREN);
    case ')':
      return SingleToken(TOKEN_RIGHT_PAREN);
    case '[':
      return SingleToken(TOKEN_LEFT_BRACKET);
    case ']':
      return SingleToken(TOKEN_RIGHT_BRACKET);
    case '{':
      return SingleToken(TOKEN_LEFT_BRACE);
    case '}':
      return SingleToken(TOKEN_RIGHT_BRACE);
    case ',':
      return SingleToken(TOKEN_LINE);
    case '@':
      return SingleToken(TOKEN_AT);
    case '.':
      return SingleToken(TOKEN_DOT);
    case '#':
      return SingleToken(TOKEN_HASH);
    case ';':
      return SingleToken(TOKEN_SEMICOLON);
    case '\\':
      return SingleToken(TOKEN_IGNORE_LINE);
    case '|':
      return SingleToken(TOKEN_PIPE);
    ...
    default:
      ...
      return Ref<Token>(new Token(
          TOKEN_ERROR, String::Format("Unrecognized character \"%c\".", c)));
    }
  }
}

bool Lexer::IsDone() const { return mNeedsLine && mReader.EndOfLines(); }
...
Ref<Token> Lexer::ReadString() {
  ...
  while (true) {
    if (IsDone())
      return Ref<Token>(new Token(TOKEN_ERROR, "Unterminated string."));

    char c = Advance();
    if (c == '"')
      return Ref<Token>(new Token(TOKEN_STRING, text));

    // An escape sequence.
    if (c == '\\') {
      if (IsDone())
        return Ref<Token>(
            new Token(TOKEN_ERROR, "Unterminated string escape."));
...
Ref<Token> Lexer::ReadName() {
  ...
  if (name == "return")
    return Ref<Token>(new Token(TOKEN_RETURN));
  if (name == "self")
    return Ref<Token>(new Token(TOKEN_SELF));
  if (name == "undefined")
    return Ref<Token>(new Token(TOKEN_UNDEFINED));
...
Ref<Token> Lexer::ReadOperator() {
  ...
  if (name == "<-")
    return Ref<Token>(new Token(TOKEN_ARROW));
  if (name == "<--")
    return Ref<Token>(new Token(TOKEN_LONG_ARROW));
