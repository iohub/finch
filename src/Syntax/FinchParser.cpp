...
Ref<Expr> FinchParser::Primary() {
  ...
  else if (Match(TOKEN_DOT))
  {
    ...
    Ref<Expr> result;
    if (LookAhead(TOKEN_LINE) || LookAhead(TOKEN_RIGHT_PAREN) ||
        LookAhead(TOKEN_RIGHT_BRACE) || LookAhead(TOKEN_RIGHT_BRACKET)) {
...
void FinchParser::ParseDefines(DefineExpr &expr, TokenType endToken) {
  while (true) {
    ...
    if (Match(endToken))
      break;
    Consume(TOKEN_LINE,
            "Definitions should be separated by commas (or newlines).");
    if (Match(endToken))
      break;
  }
}
