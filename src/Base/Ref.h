...
public:
  ...
  Ref() : mObj(NULL), mPrev(this), mNext(this) {}
  ...
  // that point on.
  explicit Ref(T *obj) : mObj(obj), mPrev(this), mNext(this) {}

  // Copies a reference. Both references will refer to the same object.
  Ref(const Ref<T> &other) : mObj(NULL), mPrev(this), mNext(this) {
    if (&other != this) {
      Link(other);
    }
  }
  ...
  // same object.
  bool operator==(const Ref<T> &other) const { return mObj == other.mObj; }
