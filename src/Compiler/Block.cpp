...
Block::Block(int methodId, const Array<String> &params)
    : mMethodId(methodId), mParams(params), mCode(), mConstants(),
      mNumRegisters(0) {}
...
void Block::Write(OpCode op, int a, int b, int c) {
  ...
  Instruction instruction =
      (op << 24) | ((a & 0xff) << 16) | ((b & 0xff) << 8) | (c & 0xff);
...
#ifdef DEBUG
void Block::DumpInstruction(Environment &environment, const String &prefix,
                            Instruction instruction) {
  using namespace std;
  ...
  case OP_MESSAGE_9:
  case OP_MESSAGE_10:
    cout << "MESSAGE_" << (op - OP_MESSAGE_0) << "   '"
         << environment.Strings().Find(a) << "' " << b << " -> " << c;
    break;
  case OP_GET_UPVALUE:
  ...
  case OP_SET_GLOBAL:
    ...
    break;
  case OP_DEF_METHOD:
    cout << "DEF_METHOD   '" << environment.Strings().Find(a) << "' " << b
         << " -> " << c;
    break;
  case OP_DEF_FIELD:
    cout << "DEF_FIELD    '" << environment.Strings().Find(a) << "' " << b
         << " -> " << c;
    break;
  case OP_END:
    cout << "END          " << a;
