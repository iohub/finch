...
public:
  ...
  virtual void Error(String message) { mInterpreter.GetHost().Error(message); }

private:
  Interpreter &mInterpreter;
};

Interpreter::Interpreter(IInterpreterHost &host) : mHost(host) {
...
String Interpreter::FindString(StringId id) { return mStrings.Find(id); }
...
const Value &Interpreter::GetGlobal(int index) { return mGlobals[index]; }
